...
- **Refinement Dispatcher Payload**: Dispatcher kini hanya mengirim field `payload` sebagai JSON body pada request HTTP, bukan seluruh objek pesan.
- **Traceability Headers Otomatis**: Sistem secara otomatis menyertakan header `X-Message-ID` untuk memudahkan pelacakan pesan di sistem target.
...
- **Penguatan Infrastruktur Pengujian (Mock Server)**: Update *mock server* internal dan *standalone* untuk menangkap, mencatat, dan memverifikasi header/metadata serta struktur body yang diterima.
